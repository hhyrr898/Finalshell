...
function isPostIndexable(data, inputPath) {
  ...
  if (/\/blog\/finalshell-[a-z0-9-]+\.md$/i.test(filePath)) return true;
...
export default function (eleventyConfig) {
  ...
  eleventyConfig.addCollection("homepagePosts", (collectionApi) => {
    const posts = collectionApi
      ...
      .filter((item) => isPostIndexable(item.data, item.inputPath));
    const pillars = posts
      .filter((item) => /\/blog\/finalshell-[a-z0-9-]+\.md$/i.test(item.inputPath))
  ...
  eleventyConfig.addCollection("publicCategories", (collectionApi) => {
    ...
    collectionApi
      ...
      .filter((item) => isPostIndexable(item.data, item.inputPath))
      .forEach((item) => {
        if (item.data.category) categories.add(String(item.data.category));
      });
